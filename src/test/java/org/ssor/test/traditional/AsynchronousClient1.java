...
public class AsynchronousClient1 {
	...
	public static void run(Replica replica, int threads) {
		...
		AsynchronousClient1.threads = threads;
		count = 0;
		ii = 0;
		try{
		main(null);
	...
	private static class ThreadTest extends Thread {
		...
		public void run() {
			...
			*/
			...
			synchronized(lock){
				count++;
				AsynchronousClient1.ii += end;
				if(AsynchronousClient1.threads == count){
					VisualizedChart.record("Service#1 ", AsynchronousClient1.ii
							/ AsynchronousClient1.threads, String.valueOf(threads));
					 System.out.print("node#1 time: "+ AsynchronousClient1.ii/AsynchronousClient1.threads+ " ms\n");
				}
			}
