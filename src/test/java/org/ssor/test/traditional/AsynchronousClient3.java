...
public class AsynchronousClient3 {
	...
	public static void run(Replica replica, int threads) {
		...
		AsynchronousClient3.threads = threads;
		count = 0;
		ii = 0;
		try{
		main(null);
	...
	private static class ThreadTest extends Thread {
		...
		public void run() {
			...
			*/
			...
			synchronized(lock){
				count++;
				AsynchronousClient3.ii += end;
				if(AsynchronousClient3.threads == count){
					VisualizedChart.record("Service#3 ", AsynchronousClient3.ii
							/ AsynchronousClient3.threads, String.valueOf(threads));
					 System.out.print("node#3 time: "+ AsynchronousClient3.ii/AsynchronousClient3.threads+ " ms\n");
				}
			}
